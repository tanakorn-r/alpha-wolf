...
Stripe AI-credit checkout uses hosted Checkout Sessions. Configure `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, and `APP_URL` on the API only; never expose the secret or webhook key through `VITE_*`. Register `POST /api/auth/stripe/webhook` for `checkout.session.completed` and `checkout.session.async_payment_succeeded`. Local webhook testing can use `stripe listen --forward-to localhost:8000/api/auth/stripe/webhook`; put the emitted `whsec_...` value in the local API environment. Credit fulfillment is idempotent by Checkout Session ID.
