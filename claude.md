...
**Last Change**
- Fixed Stock Detail showing `$0.00` despite populated historical/technical data: quote modules can contain metadata but omit price, so the old readiness test accepted a zero quote. Detail bundles now require both positive price and history; when only the quote is missing they use the latest stored native-market daily close and its native currency label (`THB` for `.BK`) without Yahoo access or THB/USD conversion, tagged `priceSource=stored_history_close`. Added Thai and real-quote preservation tests.
...
**Next Steps**
- (open) Deploy the API + frontend together, then verify Analyst timing logs on a cold and warm ticker; expected warm market-data preparation is sub-second, with remaining latency isolated to OpenAI.
