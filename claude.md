...
**Last Change**
- Fixed Stock Detail's forever-loading HTTP-200 loop: `build_detail_bundle` cached its valid `dataPending:true` placeholder for the full 180-second detail TTL, so Refresh could not observe database rows written seconds later. Pending bundles now cache for 2 seconds while ready bundles retain 180 seconds; `StockDetailDrawer` polls every 3 seconds, stops automatically when real data arrives, and changes to an explicit retry state after one minute. Added TTL regression tests; focused API tests and the Vite production build pass. Existing async Yahoo cache tests still assume the older blocking fetch contract and need updating separately.
...
**Next Steps**
- (open) Update `test_yahoo_persistent_cache.py` for the cache-first/background-refresh contract; its two fetch-count assertions still assume synchronous Yahoo calls and currently fail despite the focused pending-detail/auth tests passing.
