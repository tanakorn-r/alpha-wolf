...
export function AgentCall({
  ...
  return (
    <AgentCard agent={agent} label={label} detail={bylineDetail} accent={color} className={`shadow-[0_28px_80px_rgba(0,0,0,0.28)] ${density === "compact" ? "!p-4" : ""}`}>
      <div className={`absolute ${density === "compact" ? "right-4 top-4" : "right-5 top-4"} max-[560px]:relative max-[560px]:right-auto max-[560px]:top-auto max-[560px]:mb-4`}>
        {safeScore !== null ? (
          <div className={`relative ${density === "compact" ? "h-12 w-12" : "h-14 w-14"}`}>
            <Ring score={safeScore} color={color} size={density === "compact" ? 48 : 56} stroke={density === "compact" ? 5 : 6} />
            <div className={`absolute inset-0 grid place-items-center font-mono font-bold ${density === "compact" ? "text-[15px]" : "text-[17px]"}`} style={{ color }}>{Math.round(safeScore)}</div>
          </div>
        ) : null}
      </div>

      <div className="max-w-[860px] pr-16 max-[560px]:pr-0">
        ...
        {safeScore !== null && density !== "compact" ? <div className="mt-2 font-mono text-[9px] uppercase tracking-[0.08em] text-[#5a5a62]">{scoreLabel} · {Math.round(safeScore)}/100</div> : null}
      </div>
