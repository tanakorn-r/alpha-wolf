...
export function PortfolioPerformanceChart({ data, loading, error, onRetry, children }: { data?: PortfolioDashboard; loading: boolean; error: boolean; onRetry: () => void; children?: React.ReactNode }) {
  ...
  return <ResponsiveContainer width="100%" height="100%"><AreaChart data={data.chart} margin={{ top: 20, right: 10, bottom: 0, left: 4 }}><defs><linearGradient id="cadencePortfolio" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#3ecf8e" stopOpacity={0.28}/><stop offset="100%" stopColor="#3ecf8e" stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke="#242429" strokeDasharray="3 4" vertical={false}/><XAxis dataKey="date" tick={{ fill: "#5a5a62", fontSize: 10 }} axisLine={false} tickLine={false} minTickGap={48}/><YAxis tickFormatter={tickMoney} tick={{ fill: "#5a5a62", fontSize: 10 }} axisLine={false} tickLine={false} width={44}/><Tooltip contentStyle={tooltipStyle} formatter={(value) => formatMoneyBaht(Number(value))}/><Area type="monotone" dataKey="value" stroke="#3ecf8e" strokeWidth={2} fill="url(#cadencePortfolio)"/><Line type="monotone" dataKey="cost" stroke="#5a5a62" strokeDasharray="5 5" dot={false}/>{children}</AreaChart></ResponsiveContainer>;
}
...
export function DcaPerformanceChart({ data }: { data: PortfolioDashboard }) {
  ...
  return <ResponsiveContainer width="100%" height="100%"><AreaChart data={points}><CartesianGrid stroke="#242429" strokeDasharray="3 4" vertical={false}/><XAxis dataKey="date" hide/><YAxis hide/><Tooltip contentStyle={tooltipStyle} formatter={(value) => formatMoneyBaht(Number(value))}/><Area type="monotone" dataKey="value" stroke="#3ecf8e" fill="rgba(62,207,142,.08)"/><Line type="monotone" dataKey="dca" stroke="#f5c451" strokeWidth={0} dot={{ r: 4, fill: "#f5c451" }} connectNulls={false}/></AreaChart></ResponsiveContainer>;
}
