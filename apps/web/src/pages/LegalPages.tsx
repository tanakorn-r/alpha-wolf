...
export function PrivacyPage() {
  return <LegalPage title="Privacy Policy" intro="What we store, why we store it, and how to take it with you or delete it.">
    ...
    <Section title="Operational metrics">AlphaWolf records first-party operational counts for page usage, workflow success or failure, request status, loading time, and end-to-end duration. Reports are immediately folded into daily aggregates; we do not store a telemetry account ID, session ID, IP address, user agent, full URL, ticker search, holdings, form input, AI request content, or a raw sequence of activity in the application telemetry table.</Section>
    <Section title="Processors">Necessary data may be handled by Google for sign-in, Stripe for payments, OpenAI for requested AI analysis, and our hosting/database providers. Hosting providers necessarily process network metadata to deliver requests, but AlphaWolf does not add that metadata to its operational telemetry aggregates. We do not sell personal information or use portfolio data to execute trades.</Section>
