...
export function BuyTimingTab({ hunt }: { hunt: HuntAi }) {
  ...
  if (row.failed) return <RetryPanel label={row.error || `Could not load ${row.symbol} timing data.`} onRetry={row.retry} />;
    ...
    <div className="flex flex-col gap-3">
      ...
      <section className="rounded-[var(--aw-radius-card)] border border-[#2a2a31] bg-[#161619] p-3.5">
        <div className="flex flex-wrap items-baseline justify-between gap-3">
          <div className="text-[15px] font-bold">{timing.agentMonthlyPlan?.length ? `${timing.agent?.name ?? "Agent"}'s monthly plan` : "Seasonality evidence by month"}</div>
  ...
  return (
    <div className="mt-5">
      <div className="grid grid-cols-6 gap-1.5 min-[720px]:grid-cols-12">
        {map.map((month) => <MonthCell key={month.month} month={month} year={timing.comparisonYear ?? new Date().getFullYear()} evidenceOnly={evidenceOnly} />)}
      </div>
      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-[#8c8c95]">
        ...
        <span className="ml-auto">{evidenceOnly ? "Refresh to generate the selected Agent’s executable plan" : "Each buy uses that % of this month’s delegated DCA budget"}</span>
      </div>
    </div>
  );
}
...
function MonthCell({ month, year, evidenceOnly = false }: { month: MonthCellData; year: number; evidenceOnly?: boolean }) {
  ...
  const sizeLabel = "buyBudgetPct" in month && month.buyBudgetPct > 0 ? `${month.buyBudgetPct}% monthly` : "trimPositionPct" in month && month.trimPositionPct > 0 ? `${month.trimPositionPct}% position` : null;
  ...
  return (
    <section>
      ...
      <div className="mt-2 grid gap-1.5 min-[620px]:grid-cols-3">
        ...
        />
        <MiniStat
          label="Return · 100% exposure"
          value={result.exposureNormalizedReturnPct == null ? "—" : `${signed(result.exposureNormalizedReturnPct)}%`}
          hint="Plan return divided by average stock exposure. This is an estimate, not a simulated fully invested result."
        />
        <MiniStat
          label={`DCA · same ${exposure.toFixed(0)}% exposure`}
          value={result.matchedExposureBenchmarkReturnPct == null ? "—" : `${signed(result.matchedExposureBenchmarkReturnPct)}%`}
          hint="Full-DCA return scaled to the Agent's average stock exposure so cash level does not dominate the comparison."
        />
      </div>
      <div className="mt-2 text-[10px] leading-[1.5] text-[#5a5a62]">
        {result.edgePct < 0 && cashPct > 5
          ...
          : `The plan deployed in ${result.investedMonths} of ${result.observedMonths} months. Held shares revalue across the full history, while unused and trimmed money remains available cash.`} {`A “25%” buy invests one quarter of that month's delegated contribution only. It does not sweep cash left by earlier HOLD months. The ending ${cashPct.toFixed(0)}% is the remaining reserve divided by the final account value.`}
      </div>
      {(result.agentDividendsReceived > 0 || result.alwaysBuyDividendsReinvested > 0) ? (
        <div className="mt-2 rounded-[9px] border border-[#3ecf8e]/20 bg-[#3ecf8e]/[0.05] px-3 py-2.5 text-[10.5px] leading-[1.55] text-[#a9a9b2]">
          <b className="text-[#3ecf8e]">Dividends added {result.strategyDividendReturnBoostPct.toFixed(1)} points to the Agent’s return.</b> Without dividends the plan returned {signed(result.strategyReturnWithoutDividendsPct)}%; with accumulated dividends it returned {signed(result.strategyReturnPct)}%. The Agent received dividends equal to {agentDividendPct.toFixed(1)}% of contributed capital and kept them as cash reserve. DCA reinvestment added {result.alwaysBuyDividendReturnBoostPct.toFixed(1)} return points. Gross payments are larger than return benefit because contributions arrived gradually and some dividend cash stayed undeployed.
        </div>
      ) : null}
