...
import { colorForTone, formatCompact, normalizeSignal } from "./lib";
...
export function AnalystPanels({ detail, analysis }: { detail: StockDetailResponse; analysis: StockAnalysisResponse }) {
  ...
  const verdictColor = colorForTone(analysis.tone);
  ...
  const epsGrowth = biz?.earningsGrowth;
  const revGrowth = biz?.revenueGrowth;
  const revGrowthColor = revGrowth != null ? (revGrowth > 0 ? "#3ecf8e" : "#f2575c") : "#ececee";

  const grossMargin = biz?.grossMargin;
  ...
  return (
    <div className="grid grid-cols-2 gap-[11px] max-[900px]:grid-cols-1">
      ...
      <div className={`${panel} p-4`}>
        ...
        <div className="flex flex-col gap-[5px] border-t border-[#1d1d22] pt-[9px]">
          <div className="flex justify-between text-[11.5px]"><span className="text-[#666670]">Revenue Growth</span><span className="font-medium" style={{ color: revGrowthColor }}>{revGrowth != null ? `${revGrowth >= 0 ? "+" : ""}${(revGrowth * 100).toFixed(1)}%` : "—"}</span></div>
          <div className="flex justify-between text-[11.5px]"><span className="text-[#666670]">Net Margin</span><span className="font-medium text-[#ececee]">{biz?.profitMargin != null ? `${(biz.profitMargin * 100).toFixed(1)}%` : "—"}</span></div>
          <div className="flex justify-between text-[11.5px]"><span className="text-[#666670]">Dividend Yield</span><span className="font-medium text-[#ececee]">{biz?.dividendYield != null ? `${(biz.dividendYield * 100).toFixed(2)}%` : "—"}</span></div>
        </div>
      </div>
      ...
      <div className={`${panel} p-4`}>
        <div className="mb-3 flex items-center gap-[7px]">
          ...
          <div className="ml-auto rounded-[5px] border border-[#252529] bg-white/[0.04] px-[7px] py-[2px]">
            <span className="text-[9.5px] font-bold" style={{ color: grossMargin != null ? (grossMargin > 0.4 ? "#3ecf8e" : grossMargin > 0.2 ? "#f5c451" : "#f2575c") : "#8c8c95" }}>
              {grossMargin != null ? (grossMargin > 0.4 ? "HIGH MARGIN" : grossMargin > 0.2 ? "MID MARGIN" : "THIN MARGIN") : "REVIEW"}
            </span>
          </div>
        </div>
        <div className="mb-3">
        ...
        <div className="flex flex-col gap-[5px] border-t border-[#1d1d22] pt-[9px]">
          <div className="flex justify-between text-[11.5px]"><span className="text-[#666670]">Operating Margin</span><span className="font-medium text-[#ececee]">{biz?.operatingMargin != null ? `${(biz.operatingMargin * 100).toFixed(1)}%` : "—"}</span></div>
          ...
          <div className="flex justify-between text-[11.5px]"><span className="text-[#666670]">Beta</span><span className="font-medium text-[#ececee]">{biz?.beta != null ? biz.beta.toFixed(2) : "—"}</span></div>
        </div>
      </div>
      ...
      <div className={`${panel} relative col-span-2 max-[900px]:col-span-1 overflow-hidden p-5`}>
        ...
        <div className="relative flex items-center">
          ...
          <div className="flex flex-1 flex-col justify-center gap-[5px] pl-7">
            ...
            <div className="text-[22px] font-extrabold tracking-[-0.3px]" style={{ color: verdictColor }}>{normalizeSignal(analysis.signal)}</div>
