            ...
            <div className="mb-3 border-l-2 pl-3" style={{ borderColor: agentColor }}>
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex flex-none items-center gap-2">
                  <button type="button" onClick={() => void strategy.run(strategy.mode, true)} disabled={hunt.signedIn && (!hasFive || !tokensAvailable || strategy.candidatesLoading)} title={tokensAvailable ? "Run a fresh Agent ranking using the latest candidate data." : "No AI tokens remaining."} className="inline-flex h-7 items-center rounded-[7px] border border-white/[0.12] bg-white/[0.035] px-2.5 text-[9px] font-extrabold text-[#bcbcc2] hover:border-white/[0.22] hover:text-[#ececee] disabled:cursor-not-allowed disabled:opacity-45">{!hunt.signedIn ? "Sign in to rerank" : tokensAvailable ? "Rerank" : "No tokens"}</button>
                  <button type="button" onClick={savedOrder ? strategy.remove : strategy.save} disabled={strategy.saving || strategy.removing || strategy.picks.length !== 5} title={savedOrder ? "Remove this saved Top Five. Holdings and watchlist stocks stay unchanged." : "Save this Agent-ranked Top Five"} className="inline-flex h-7 items-center gap-1.5 rounded-[7px] border px-2.5 text-[9px] font-extrabold hover:brightness-125 disabled:opacity-55" style={{ borderColor: `${agentColor}66`, background: `${agentColor}12`, color: agentColor }}>{strategy.saving || strategy.removing ? <LoadingSpinner size={10} /> : null}{strategy.saving ? "Saving…" : strategy.removing ? "Removing…" : savedOrder ? "Remove saved" : "Save Top 5"}</button>
                </div>
              </div>
