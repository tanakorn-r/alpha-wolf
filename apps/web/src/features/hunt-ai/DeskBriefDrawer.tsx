...
export function DeskBriefDrawer({ hunt, onClose }: { hunt: HuntAi; onClose: () => void }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[90] isolate flex justify-end bg-black/70 backdrop-blur-[2px]">
      ...
      <aside ref={drawerRef} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1} className="relative flex h-[100dvh] w-full max-w-[880px] flex-col overflow-hidden border-l border-[#34343c] bg-[#101012] shadow-[-28px_0_80px_rgba(0,0,0,.55)] outline-none">
        <header className="flex flex-none items-center justify-between gap-4 border-b border-[#29292f] bg-[#151518] px-4 py-3.5 min-[640px]:px-5">
          <div className="min-w-0">
            <div className="text-[8.5px] font-bold uppercase tracking-[0.13em] text-[#3ecf8e]">Daily intelligence</div>
            <h2 id={titleId} className="mt-0.5 text-[17px] font-black tracking-[-0.25px] text-[#ececee]">Your desk brief</h2>
          </div>
