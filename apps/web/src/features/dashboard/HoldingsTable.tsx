        ...
        {dash.portfolio?.holdings.map((holding) => {
          ...
          return (
            <div key={holding.symbol} className="grid w-full grid-cols-[1fr_70px_repeat(4,minmax(72px,96px))_auto] items-center gap-3 px-5 py-3 text-left hover:bg-[#1c1c20]">
              ...
              <button type="button" onClick={() => dash.startSell(holding)} className="justify-self-end rounded-lg border border-[#2a2a31] px-3 py-1.5 text-xs text-[#8c8c95] hover:border-[#f2575c] hover:text-[#f2575c]">
                Sell
              </button>
            </div>
          );
...
function Cell({ label, value, good }: { label: string; value: React.ReactNode; good?: boolean }) {
  return (
    <div>
      ...
      <div className={`mt-1 font-mono text-sm ${good === true ? "text-[#3ecf8e]" : good === false ? "text-[#f2575c]" : ""}`}>{value}</div>
    </div>
  );
}
