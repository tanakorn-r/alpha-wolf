...
export function StockDetailDrawer() {
  ...
  return (
    <>
      ...
      <aside ref={drawerRef} onClick={closeDetail} className={`aw-drawer fixed inset-0 z-40 flex items-end justify-center overflow-hidden px-0 pb-[var(--aw-safe-bottom)] pt-[var(--aw-safe-top)] transition-opacity min-[720px]:items-start min-[720px]:overflow-y-auto min-[720px]:px-6 min-[720px]:py-8 ${detailOpen ? "opacity-100" : "pointer-events-none opacity-0"}`} aria-label="Stock detail panel">
        <div ref={dialogRef as React.Ref<HTMLDivElement>} role="dialog" aria-modal="true" aria-label={`${selectedSymbol} stock detail`} tabIndex={-1} onClick={(event) => event.stopPropagation()} className="aw-detail-panel relative flex max-h-[calc(100dvh-var(--aw-safe-top)-var(--aw-safe-bottom))] min-w-0 w-full max-w-[1040px] flex-col overflow-hidden rounded-t-[var(--aw-radius-frame)] border border-[#2a2a31] bg-[#0e0e10] shadow-[0_30px_90px_rgba(0,0,0,.55)] outline-none min-[720px]:max-h-none min-[720px]:rounded-[var(--aw-radius-frame)]">
          <DrawerHeader
            detail={detail}
